function [res] = chain_phase_oscil(a,e,Noscils)
...
if nargin<2
    e=-0.06;  % Difference of intrinsic frequencies from one oscillator to the next. FEEL FREE TO CHANGE
end
if nargin<3
...
%omega=pi*[2,1.6,1.3,1.1,1,0.95,0.9,0.8,0.6,0.3]';
...
theta0  = 2*pi*(rand(Noscils,1)-0.5); % Random initial conditions
theta = theta0;
counter = 0;
...
for t=0:dt:t_end
    ...
    % 6.C
    phasefirst = mod(theta(1),2*pi);
    phaselast = mod(theta(end),2*pi);
    phasedifff = phaselast-phasefirst;
    
    if (phasedifff > -0 && phasedifff < pi)
        a_sens = 5;
        omega_mech = 1.7*pi/(1.05+phasedifff);
    else
        a_sens = 0;
        omega_mech = 1.7*pi;
    end
    ...
    % Euler integration
    theta = mod(theta + dtheta*dt,2*pi);
...
figure(4)
plot(1:20,mod(mechtheta1(1)-mechtheta1,2*pi),1:20,mod(mechtheta2(1)-mechtheta2,2*pi));%,1:20,mod(theta(1),2*pi)-mod(theta,2*pi));
ylabel('Stable phase difference with oscillator 1');
xlabel('Oscillator');
legend('with sensory input','without sensory input','Location','southeast');

 res = {fixed_points,omega,resulting_freq',X_shifted};
 size(res);
